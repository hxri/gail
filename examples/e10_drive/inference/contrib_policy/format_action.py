...
def _relative_target_pose() -> Tuple[Callable[[int, float], np.ndarray], gym.Space]:
    ...
    def wrapper(action: int, prev_heading: float) -> np.ndarray:
        ...
        # jibiyui
        # dffsdf
        # print([action_map[key] for key in action.tolist()])
        # print(action_map[action.item()][0])
        # print(action_map[action.item()][1])
        # mp = [action_map[key] for key in action.tolist()]
        # magnitude = [i[0] for i in mp]
        # delta_heading = [i[1] for i in mp]
        # action = action.tolist()

        # map = [action_map.get(a) if action_map.get(a) else a for a in action.tolist()]
        print(action_map[action.item()])
        magnitude = action_map[action.item()][0]
        delta_heading = action_map[action.item()][1]
        # magnitude = [i[0] for i in map] # action_map[action][0]
        # delta_heading = [i[1] for i in map] # action_map[action][1]

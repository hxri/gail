...
class Reward(gym.Wrapper):
    ...
    def step(self, action):
        ...
        for agent_id, agent_obs in obs.items():
            ...
            # print(np.shape(agent_obs['drivable_area_grid_map']))
            cv2.imshow('TDV_RGB', agent_obs['top_down_rgb'])
            cv2.imshow('OCCG', agent_obs['occupancy_grid_map'])
            cv2.imshow('DRV_A', agent_obs['drivable_area_grid_map'])
            cv2.waitKey(1)

            # print(obs['Agent_0'].keys())
            # print(obs['Agent_0']['distance_travelled'])

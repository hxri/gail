...
def flatten_list_dicts(list_dicts):
    ...
    for k in list_dicts[-1].keys():
        for d in list_dicts:
            out_dict[k].append([d[k]])
    #     print(k)
    # # print(len(states))
    # print(out_dict['entropies'])
    # dfdfd
    # # print(s_cat)
...
def indicate_absorbing(states, actions, infos, next_states=None):
    absorbing_idxs = infos.to(dtype=torch.bool)
...
class TransitionDataset(Dataset):
    def __init__(self, transitions):
        super().__init__()
        transitions['states'] = torch.Tensor(transitions['states'])
        transitions['actions'] = torch.Tensor(transitions['actions']).detach()
        transitions['rewards'] = torch.Tensor(transitions['rewards'])
        transitions['infos'] = torch.Tensor(transitions['infos'])
        self.states, self.actions, self.rewards, self.infos = transitions['states'], transitions['actions'].detach(), transitions['rewards'], transitions['infos']  # Detach actions
    ...
    def __getitem__(self, idx):
        if isinstance(idx, str):
            ...
            return dict(states=self.states[idx], actions=self.actions[idx], rewards=self.rewards[idx], next_states=self.states[idx + 1], infos=self.infos[idx])

    def __len__(self):
        return self.infos.size(0) - 1  # Need to return state and next state
    
# Dataset that returns transition tuples of the form (s, a, r, s', terminal)
class TransitionDataset2(Dataset):
    def __init__(self, transitions):
        super().__init__()
        transitions['states'] = torch.Tensor(transitions['states'])
        transitions['actions'] = torch.Tensor(transitions['actions']).detach()
    ...
    def __getitem__(self, idx):
        if isinstance(idx, str):
            ...
            self.states = self.states.reshape((np.shape(self.states)[2], 1, 9, 130, 100))
            return dict(states=self.states[idx], actions=self.actions[idx], rewards=self.rewards[idx], next_states=self.states[idx + 1], infos=self.infos[idx])
...
def compute_advantages(trajectories, next_value, discount, trace_decay):
    with torch.no_grad():  # Do not differentiate through advantage calculation
        ...
        for t in reversed(range(len(trajectories['states']))):
            reward_to_go = trajectories['rewards'][t] + (1 - trajectories['infos'][t][0]) * (discount * reward_to_go)  # Reward-to-go/value R
            trajectories['rewards_to_go'][t] = reward_to_go
            if(np.shape(trajectories['values'][t]) == (1,)):
                td_error = trajectories['rewards'][t].item() + (1 - trajectories['infos'][t][0])[0].item() * discount * next_value[0].detach().item() - trajectories['values'][t][0].detach().item() # TD-error δ
            else:
                td_error = trajectories['rewards'][t] + (1 - trajectories['infos'][t][0]) * discount * next_value - trajectories['values'][t]
            advantage = td_error + (1 - trajectories['infos'][t][0]) * discount * trace_decay * advantage  # Generalised advantage estimate ψ
...
def ppo_update(agent, trajectories, agent_optimiser, ppo_clip, epoch, value_loss_coeff=1, entropy_reg_coeff=1):
    ...
    if epoch > 0:
        ...
        trajectories['log_prob_actions'], trajectories['entropies'] = policy.log_prob(trajectories['actions']), policy.entropy()
    ...
    value_loss = F.mse_loss(trajectories['values'], trajectories['rewards_to_go'].squeeze())  # Fit value function by regression on mean squared error
    ...
    entropy_reg.requires_grad_(True)
...
def target_estimation_update(discriminator, expert_trajectories, discriminator_optimiser, batch_size, absorbing):
    ...
    for expert_transition in expert_dataloader:
        ...
        if absorbing: expert_state, expert_action = indicate_absorbing(expert_state, expert_action, expert_transition['infos'])
...
def adversarial_imitation_update(discriminator, expert_trajectories, policy_trajectories, discriminator_optimiser, batch_size, absorbing=False, r1_reg_coeff=1, pos_class_prior=1, nonnegative_margin=0):
    ...
    for expert_transition, policy_transition in zip(expert_dataloader, policy_dataloader):
        expert_state, expert_action, expert_next_state, expert_terminal = expert_transition['states'], expert_transition['actions'], expert_transition['next_states'], expert_transition['infos']
        policy_state, policy_action, policy_next_state, policy_terminal = policy_transition['states'], policy_transition['actions'], policy_transition['next_states'], policy_transition['infos']
        ...
        # print(expert_action)
        expert_state = expert_state.squeeze(1)
        policy_state = policy_state.squeeze(1)
